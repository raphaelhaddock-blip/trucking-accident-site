...
## PR3 LOG (pro image system on every page + index hubs + mobile excerpt)
- NO image generator in-session (only SVG/HTML raster) — built the auto-wiring system + generation pack instead; did NOT fake-wire missing files.
- Image system: scripts/gen-image-manifest.ts (npm `prebuild`) scans public/brand/photo → src/lib/brand-images.generated.ts; src/lib/brand-images.ts resolver (heroPhoto: city→state→index-motif→global→null=command treatment, never Sanity; ogImage: specific→og-default, never Sanity). Drop a correctly-named .webp → next build auto-wires by slug.
- Wired imageSrc on home/contact/state/city/accident CommandHero; swapped per-page OG + JSON-LD image in all 3 templates from Sanity STATE_IMAGES/ACCIDENT_IMAGES to local resolver; removed dead DEFAULT_OG_IMAGE consts + STATE_IMAGES/ACCIDENT_IMAGES imports; removed dead cdn.sanity.io preconnect in layout.
- INDEX HUBS redesigned (were still old navy/gray): states/page.tsx + accidents/page.tsx → CommandHero+Section+Icon (2 parallel agents, clean tsc/eslint, canonicals /states + /accidents preserved). states-index visually confirmed on-system + consistent.
- CommandHero subtitle mobile excerpt: `[&>p:nth-child(n+2)]:hidden sm:...:block` (1st para on mobile, full ≥sm; all in DOM).
