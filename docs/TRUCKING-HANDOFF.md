...
1. **Finish the P4 city-hub image *plan*** — the plan, not the generation. **DONE 2026-06-25:** [P4-CITY-HUB-IMAGE-PLAN.md](P4-CITY-HUB-IMAGE-PLAN.md) is now final/approval-ready — 10 cities in 2 tiers, exact per-city prompts, render target, ~$0.40–0.50 cost, pass/fail + tier gate. **Prep fix DONE 2026-06-25:** the `CITY_SCENE` prompt-builder fix + `--dry-run` mode landed in `scripts/generate-brand-images.ts` and all 10 prompts were dry-run-verified (no slug-mangling, 10 distinct, safety-clean, zero API calls). **Tier A GENERATED + PASSED 2026-06-25** — 5 images (Phoenix, Salt Lake City, Miami, Los Angeles, Kansas City), ~$0.20–0.25, 0 rejected, built + hero/OG-wired, `audit:legaltone` PASS, all distinct (clears Tier B gate). Artifacts in `docs/redesign-assets/after-p4-tierA/`. **Tier B GENERATED + PASSED 2026-06-25** — 5 images (Dallas, Chicago, Atlanta, NYC, Philadelphia), ~$0.20–0.25, 0 rejected, built + hero/OG-wired (NYC & Chicago verified), `audit:legaltone` PASS. Artifacts in `docs/redesign-assets/after-p4-tierB/`. **P4 / city image system COMPLETE — 11 city heroes.** No further skyline cities (Dallas/Atlanta already double the aerial-interchange look); add a one-off city image later only if a specific high-value page needs it.

**Preview QA 2026-06-25 (PR #1):** preview built green on `trucking-accident-site` (behind Vercel SSO). Build + `audit:legaltone` PASS; SEO clean (canonical/schema/sitemap/robots, 0 Sanity refs); desktop visual PASS (7 pages, 0 broken images / 0 overflow). **Two gates left → [PREVIEW-QA-2026-06-25.md](PREVIEW-QA-2026-06-25.md):** (1) **real phone number** — placeholder `1-800-555-0123` in 14 consts + 26 lines; Raphy chose 2026-06-25 to keep it for this review round → HARD pre-production gate; (2) **mobile pixel** pass — blocked in-tool (window won't go <1512px; SSO blocks Playwright), responsive layer DOM-verified, needs a human/device-mode eyeball. No merge/deploy.
2. **Decide whether to deploy the redesign** later to the correct `trucking-accident-site` project. This is a human call (see §11). If yes, link the worktree to that project deliberately and re-run build + tone audit first. **→ Read the [Final Redesign Readiness Packet](FINAL-REDESIGN-READINESS-PACKET.md) first** — commit range, image inventory, verification, deploy sequence, and stop conditions for the deploy review.
...
4. **Growth OS** — ✅ foundation BUILT 2026-06-25 (recommendation-only): taxonomy/ontology model + rankability gate + learning ledger + ontology interlink + `npm run growth:daily` dry-run, verified on 1695 pages. Spec: [GROWTH-OS-MASTER-SPEC.md](GROWTH-OS-MASTER-SPEC.md). Net-new content is refused until real demand (GSC) is wired — that wiring (paid/real source) is the next gated step, and content scaling stays blocked behind it + human approval.
